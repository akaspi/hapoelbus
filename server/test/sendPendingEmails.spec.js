...
fdescribe('sendPendingEmails spec', () => {
  ...
  describe('pending templates', () => {
    ...
    it('should remove mail from db after success sending', done => {
      ...
      sendPendingEMails.exec()
        .then(() => {
          expect(db.remove).toHaveBeenCalledWith('pendingEmails/templates/mailId');
          done();
        });
    });
  ...
  describe('pending custom mails', () => {
    ...
    it('should remove mail from db after success sending', done => {
      ...
      sendPendingEMails.exec()
        .then(() => {
          expect(db.remove).toHaveBeenCalledWith('pendingEmails/custom/mailId');
          done();
        });
    });
    ...
    it('should send all', done => {
      ...
      sendPendingEMails.exec()
        .then(() => {
          ...
          expect(db.remove.calls.argsFor(0)).toEqual(['pendingEmails/templates/mailId']);
          expect(db.remove.calls.argsFor(1)).toEqual(['pendingEmails/custom/mailId']);
          done();
        });
    });
