...
describe('sendPendingSMS spec', () => {
  ...
  it('should append +972 to phone numbers by default', done => {
    ...
    db.read.and.returnValue(Promise.resolve({
      smsId: {
        to: ['0501234567'],
  ...
  it('should remove sms from pending list', done => {
    ...
    sendPendingSMS.exec()
      .then(() => {
        expect(db.remove).toHaveBeenCalledWith('pendingSMS/smsId1');
        done();
      });
  });
  ...
  describe('send sms failed', () => {
    ...
    it('should not remove pending sms', done => {
      ...
      sendPendingSMS.exec()
        .then(() => {
          expect(db.remove).not.toHaveBeenCalledWith('pendingSMS/smsId1');
          done();
        });
    });
    ...
    it('should replace the numbers to send in DB with the numbers that failed to receive sms', done => {
      ...
      sendPendingSMS.exec()
        .then(() => {
          expect(db.remove).not.toHaveBeenCalledWith('pendingSMS/smsId1');
          expect(db.setIn).toHaveBeenCalledWith('pendingSMS/smsId1/to', ['0501234566', '0501234567', '0501234569']);
          done();
        });
    });
    ...
    it('should not change the numbers to send if all of the numbers failed to receive sms', done => {
      ...
      sendPendingSMS.exec()
        .then(() => {
          expect(db.remove).not.toHaveBeenCalledWith('pendingSMS/smsId1');
          expect(db.setIn).not.toHaveBeenCalledWith('pendingSMS/smsId1/to');
          done();
        });
    });
